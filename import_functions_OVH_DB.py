...
def initiate_threaded_connection_pool(db_package):
    ...
    try:
        global conn_pool
        conn_pool = psycopg2.pool.ThreadedConnectionPool(1,20, connect_token)
...
def extract_arguments(file_name, silent = False):
    ...
    if file_arguments:
        if len(file_arguments) == 3:
            ...
            try:
                parsed_date = dparser.parse(file_date, fuzzy = True, dayfirst = True)
...
"""
def get_additional_DB_connection(db_package = ""):
    connect_token = "dbname='" + db_package[0] + "' user='" + db_package[1] + \
                    "' host='" + db_package[2] + "' password='" + db_package[3] + "'"
    if 'additional_DB_conn' in globals():
        try:
            additional_DB_conn.close()
        except:
            global additional_DB_conn
            try:
                additional_DB_conn = psycopg2.connect(connect_token)
            except:
                message = "Pb. Failed to open additional connection with DB."
                print_to_log(log_file, 2, message)
                return [False]
    else:
        global additional_DB_conn
        try:
            ...
            return [False]
    try:
        global additional_DB_cursor
        additional_DB_cursor = direct_DB_conn.cursor()
        return [True, additional_DB_conn, additional_DB_cursor]
    except:
        message = "Pb. Failed to create additional cursor with DB -- although connection exists."
        print_to_log(log_file, 2, message)
        return [False]

def close_additional_DB_connection():
    if 'additional_DB_cursor' in globals():
        additional_DB_cursor.close()
    if 'additional_DB_conn' in globals():
        additional_DB_conn.close()
"""
...
def insert_provenance(fichier_num, db_schema = ""):
    try:
        ...
        try:
            if db_schema:
                direct_DB_cursor.execute("SELECT id FROM " + db_schema + ".fichier_list WHERE fichier_num = %s", \
                                         (str(fichier_num),))
            else:
                direct_DB_cursor.execute("SELECT id FROM fichier_list WHERE fichier_num = %s", \
                                         (str(fichier_num),))   
        except:
...
def modifications_on_mail_syntax(dataframe):
    ...
    #print "debug dataframe['mail']"
    try:
        dataframe['mail'] = dataframe['mail'].apply(lambda x: str(unidecode(x)))
    except:
        df_index = dataframe.index
        mail_str_list = []
        for cpt in range(len(df_index)):
            raw_mail = dataframe.at[df_index[cpt], 'mail']
            try:
                mail_str = str(unidecode(raw_mail))
            except:
                mail_str = ""
            mail_str_list.append(mail_str)
        dataframe['mail'] = mail_str_list
...
def sync_md5_table(DB_conn, csv_path = path_containing_all_csv):
    query = "SELECT base.id AS mail_id, base.mail FROM base " + \
            "WHERE NOT EXISTS (SELECT 1 FROM md5 WHERE md5.mail_id = base.id) ORDER BY mail_id ASC;"
    with getconnection() as thread_connection:
        missing_md5 = sql.read_sql(query, thread_connection, coerce_float=False)
...
def test_md5_table_sync(DB_conn):
    ...
    if nb_mail > nb_md5:
        sync_md5_table(DB_conn, csv_path = path_containing_all_csv)
...
def load_md5_table(DB_conn, limit = "", silent = False):
    ...
    try:
        with getconnection() as thread_connection:
            md5 = pd.read_sql(query, thread_connection)
...
def remove_floating_part_mindbaz(value):
    num_sep = [",", "."]
    for item in num_sep:
        if item in value:
            pos = value.find(item)
...
def format_civilite_for_criteo(value):
    select_case = {'NaN' : "", 'nan' : "", '1.0' : 'Mr.', '2.0' : 'Mme', '3.0' : 'Mme'}
    str_value = str(value)
    ...
    if len(dataframe.index) > 0:
        for col in write_cols:
            if "id" in col:
                ...
                dataframe[col] = dataframe[col].apply(str)
                if "mindbaz" in col:
                    dataframe[col] = dataframe[col].apply(lambda x: remove_floating_part_mindbaz(x))
                if "criteo" in col:
                    dataframe[col] = dataframe[col].apply(lambda x: format_civilite_for_criteo(x))
                else:
                    dataframe[col] = dataframe[col].apply(lambda x: remove_floating_part(x))
    ...
    if len(dataframe.index) != 0:
        ...
        if dataframe_result[0]:
            ...
            if len(df_pg_id.index) != 0:
                df_csv = pd.DataFrame(columns = ['mail_id', 'fichier_id'], index = df_pg_id.index)
                df_csv['mail_id'] = df_pg_id
                df_csv['fichier_id'] = prov_fichier_id
                if not table:
                    table = "fichier_match"
                show_df(df_csv)
                df_csv = remove_duplicates_in_inserts_csv(df_csv, table, fichier_num, join_key = ['mail_id'])
...
fields_list_dict = {'b2b' : ['b2b', 'mimi_b2b'], 'ouvreur' : ['ouvreur', 'mimi_ouvreur'], \
                    'plainte' : ['plainte', 'mimi_plainte', 'plainte_mimi'], 'npai' : ['npai', 'base_mimi_npai'], \
                    'regie' : ['regie'], 'actif' : ['actif'], 'routeur' : ['routeur'], \
                    'optin' : ['optin'], 'desabo' : ['desabo'], 'partner' : ['partner'], \
                    'pcp' : ['pcp'], 'pcp-wl' : ['pcpwl'], 'pcp-sm' : ['pcpsm'], \
                    'ok_npai' : ['ok_npai', 'ok'], \
                    'emarsys' : ['emarsys'], 'emarsys_plainte' : ['emarsys_plainte']}
query_status_dict = {'regie' : ['regie_id', 'regie_list', 'abreviation'], \
                     'actif' : ['regie_id', 'regie_list', 'abreviation'], \
                     'optin' : ['optin_id', 'optin_list', 'abreviation'], \
                     'desabo' : ['optin_id', 'optin_list', 'abreviation'], \
                     'partner' : ['partner_id', 'partner_list', 'abreviation'], \
                     'routeur' : ['routeur_id', 'routeur_list', 'abreviation']}
table_dict = {'b2b' : 'base_b2b', 'ouvreur' : 'base_mimi_ouvreur', \
              'plainte' : 'base_mimi_plainte', 'npai' : 'base_mimi_npai', \
              'regie' : 'regie_match', 'actif' : 'regie_actif', \
              'optin' : 'optin_match', 'desabo' : 'optin_desabo', 'routeur' : 'routeur_match', \
              'partner' : 'partner_match', \
              'pcp' : 'base_pcp', 'pcp-wl' : 'regie_pcp_wl', 'pcp-sm' : 'regie_pcp_sm', \
              'ok_npai' : 'base_emailverifier', \
                        ...
                        for field_insert, field_list in fields_list_dict.iteritems():
                            if target in field_list:
                                ...
                                try:
                                    ...
                                    print_to_log(log_file, 2, message)
                                    return [False]
                                if target in query_status_dict.iterkeys():
                                    col_in_target_table = query_status_dict[target][0]
                                    query_table = query_status_dict[target][1]
                                    where_arg = query_status_dict[target][2]
                                    with getcursor() as quick_cursor:
                                        try:
                                            select_id_query = "SELECT id FROM %s WHERE %s = '%s'" % \
                                                            (str(query_table), str(where_arg), str(info))
                                            quick_cursor.execute(select_id_query)
                                            records = quick_cursor.fetchall()
                                        except:
                                            e = sys.exc_info()
                                            for item in e:
                                                message = str(item)
                                                print_to_log(log_file, 3, message)
                                    info_id = str(records[0]).strip('()').strip(',')
                                    df_csv[col_in_target_table] = info_id
                                if date_insert[0] and date_insert[1] != "":
...
def threading_query_duplicates_removal(fields, table, fichier_num):
    ...
    select_part = "SELECT %s.%s FROM %s " % (table, field_str, table)
    join_part = "INNER JOIN fichier_match ON fichier_match.mail_id = %s.mail_id " % (table)
    where_part = "WHERE fichier_match.fichier_id IN " + \
            "(SELECT fichier_list.id FROM fichier_list WHERE fichier_num LIKE '%s')" \
            % (str(fichier_num))
    ...
    try:
        with getconnection() as thread_connection:
            table_df = sql.read_sql(query, thread_connection, coerce_float=False)
        ...
        print_to_log(log_file, 4, message)
        show_df(table_df)
        print "Above is the dataframe stored in dupli_dict for insert in table '%s'" % str(table)
        return table_df
    except:
        e = sys.exc_info()
        for item in e:
...
def convert_raw_value_to_dict(value, arg_sep = ', ', key_value_sep = ': '):
    if value and value != "":
        if isinstance(value, basestring):
            if value != "NaN":
                new_dict = dict((int(k), int(v)) for k, v in \
            ...
            if '}' in value [-1]:
                value = value[:-1]
            value_list = list(value.split(arg_sep))
            res_dict = {}
            for value_item in value_list:
                value_item_list = list(value_item.split(key_value_sep))
                res_dict[value_item_list[0]] = value_item_list[1]
            return res_dict"""
...
def modify_csv_if_dict(df_csv, dict_field, key_field, value_field):
    ...
    for cpt in range(len(df_csv.index)):
        ...
        #print dict_arg
        try:
            dict_arg = convert_raw_value_to_dict(dict_arg)
        except:
            e = sys.exc_info()
            for item in e:
                message = str(item)
        ...
        for key, value in dict_arg.iteritems():
            ...
            record.append(value)
            data.append(record)
    new_df = pd.DataFrame(data, columns = new_fields, dtype=object)
    #show_df(new_df)
    return new_df

def clean_csv_from_empty_values(df_csv, field):
    if len(df_csv.columns) == 2:
        df_csv = df_csv[df_csv[field] != ""]
    return df_csv
...
def build_extra_fields_dataframe(dataframe, csv_fields, table, fichier_num):
    ...
    for field in csv_fields:
        ...
        if field in ["port", "tel", "tel1", "tel2"]:
            df_csv = clean_csv_from_empty_values(df_csv, field)
...
def check_mot_cle_sql(fichier_num): #42
    ...
    if db_connect[0]:
        ...
        return
    df_mot = sql.read_sql("SELECT * FROM %s" % ('mot_list'), conn, index_col='id', coerce_float=False)
    show_df(df_mot)
    ...
    show_df(df_result)
    table = "mot_match"
    export_path = path_containing_all_csv
...
def check_mot_cle_DB(million_num): #42
    db_name = "prod" #"postgres"
    db_user = "postgres"
    db_host = "localhost"
    db_pass = "penny9690"
    ...
    if million_num == 1:
        table = "mot_match"
        export_path = path_containing_all_csv
        file_name = "1_[mail_id,mot_list_id]_mot_match_45k.csv"
        csv_reader = codecs.open(export_path + "/" + table + "/" + file_name, 'r', encoding='utf-8')
        db_connect = DB_direct_connection(db_package)
        if db_connect[0]:
            db_connect[2].copy_from(file = csv_reader, table = table,
                                       sep = ";", null = "", columns = ['mail_id', 'mot_list_id'])
            db_connect[1].commit()
        ...
        select_part = "SELECT base.id, base.mail FROM base "
        #join_part = "INNER JOIN fichier_match ON fichier_match.id = base.id "
        where_part = "WHERE base.id >= '%s' AND base.id <= '%s' " \
                     % (str(((million_num - 1) * 1000000) + 1), str((million_num * 1000000)))
        ...
        for cpt in range(len(df_mail.index)):
            ...
            if check[0]:
                record = [mail_id, check[1]]
                data.append(record)
        df_result = pd.DataFrame(data, columns = ['mail_id', 'mot_list_id'], index = None)
        show_df(df_result)
        table = "mot_match"
        export_path = path_containing_all_csv
        file_name = create_file_name(million_num, table, len(df_result.index), \
                                     header = list(df_result.columns), comment = "")
        write_to_csv(df_result, export_path, table, file_name)
        db_connect = DB_direct_connection(db_package)
        if db_connect[0]:
            conn = db_connect[1]
            write_csv_to_DB('mot_match', export_path + "/" + file_name, list(df_result.columns))
            conn.close()

def write_files_in_DB(file_path, table, field_list):
    db_name = "prod" #"postgres"
    db_user = "postgres"
    db_host = "localhost"
    db_pass = "penny9690"
    ...
    for file_name in file_name_list:
        csv_reader = codecs.open(file_path + "/" + file_name, 'r', encoding='utf-8')
        with getconnection() as db_connect:
            db_connect.cursor().copy_from(file = csv_reader, table = table,
                                            sep = ";", null = "", columns = field_list)
            db_connect.commit()
        message = "OK : %s imported to DB in table : %s" % (str(file_name), str(table))
        print message

# --- clean id table ---

def clean_id_table(db_pack, sort = 'mail_id', limit = 34000000, test = "", table = 'id'):
    #global db_package
    initiate_threaded_connection_pool(db_pack)
    query = "SELECT * FROM %s WHERE id <= '%s'" % \
            (str(table), str(limit))
    with getconnection() as this_conn:
        id_df = pd.read_sql(query, this_conn, coerce_float = False)
    show_df(id_df)
    sort_field = sort
    mail_id_df = id_df[[sort_field]]
    mail_id_df = drop_null_or_missing_values(mail_id_df, sort_field)
    mail_id_df = sort_unique(mail_id_df, sort_field, sort_field)[1]
    show_df(mail_id_df)
    df_columns = list(id_df.columns)
    df_columns.append('info_sup')
    nb_unik = 0
    nb_multi = 0
    dataset = []
    milestones = {0 : 100, 1 : 1000, 2 : 2000, 3 : 5000, 4 : 10000, 5 : 20000, 6 : 50000, \
                  7 : 100000, 8 : 200000, 9 : 500000, 10 : 1000000, 11 : 2000000, 12: 5000000}
    cpt_milestones = 0
    if test != "":
        explore_len = int(test)
    else:
        explore_len = len(mail_id_df.index)
    for cpt in range(explore_len):
        if cpt == milestones[cpt_milestones]:
            message = "OK. First " + str(milestones[cpt_milestones]) + " records looked up."
            print_to_log(log_file, 4, message)
            cpt_milestones += 1
        cpt_df = mail_id_df.index[cpt]
        this_id = mail_id_df.at[cpt_df, sort_field]
        this_df = id_df[id_df[sort_field] == this_id]
        this_df_len = len(this_df.index)
        if this_df_len > 1:
            nb_multi += 1
            #print this_id, this_df_len, nb_multi
            #show_df(this_df)
            this_entry = []
            for col in df_columns[1:-1]:
                cpt_stop = 0
                while pd.isnull(this_df.at[this_df.index[cpt_stop], col]) and cpt_stop < (this_df_len - 1):
                    cpt_stop += 1
                if pd.isnull(this_df.at[this_df.index[cpt_stop], col]):
                    this_entry.append("")
                else:
                    if col == "birth":
                        this_birth = this_df.at[this_df.index[cpt_stop], col]
                        #print this_birth
                        #print clean_birth(this_birth)
                        this_entry.append(clean_birth(this_birth))
                    elif col == "civilite":
                        this_civi = this_df.at[this_df.index[cpt_stop], col]
                        this_civi = str(this_civi)[0]
                        #print this_civi
                        this_entry.append(this_civi)
                    else:
                        this_entry.append(this_df.at[this_df.index[cpt_stop], col])
            this_entry.append("True")
            #print this_entry
            ...
            this_record = []
            for col in df_columns[1:-1]:
                if pd.isnull(this_df.at[this_df.index[0], col]):
                    this_record.append("")
                else:
                    if col == "birth":
                        this_birth = this_df.at[this_df.index[0], col]
                        #print this_birth
                        #print clean_birth(this_birth)
                        this_record.append(clean_birth(this_df.at[this_df.index[0], col]))
                    elif col == "civilite":
                        flag_civi = True
                        this_civi = this_df.at[this_df.index[0], col]
                        this_civi = str(this_civi)[0]
                        #print this_civi
                        this_record.append(this_civi)
                    else:
                        this_record.append(this_df.at[this_df.index[0], col])
            this_record.append("False")
            #if flag_civi: print this_record
            dataset.append(this_record)
    print nb_unik, nb_multi
    #for record in dataset: print record
    new_df = pd.DataFrame(data = dataset, columns = df_columns[1:], dtype = object)
    show_df(new_df)
    table = "id_unified"
    export_path = path_containing_all_csv
    file_name = create_file_name('999', table, len(new_df.index), \
                                 header = list(new_df.columns), comment = "")
    write_to_csv(new_df, export_path, table, file_name)

def write_file_in_DB(db_package, complete_path_to_csv, table, field_list):
    csv_reader = codecs.open(complete_path_to_csv, 'r', encoding='utf-8')
    initiate_threaded_connection_pool(db_package)
    with getconnection() as db_connect:
        db_connect.cursor().copy_from(file = csv_reader, table = table,
                                        sep = ";", null = "", columns = field_list)
        db_connect.commit()
    message = "OK : %s imported to DB in table : %s" % (str(complete_path_to_csv), str(table))
    print message

def unify_id_table(db_package):
    query = "SELECT " + \
            "i.mail_id," + \
...
def make_into_encoding(input, encoding = 'utf-8'):
...
def unpack_id_attribute(container_dict, attribute, raw_value, separator = ","):
    if raw_value:
        raw_value = str(raw_value)
        if raw_value.lower() != "nan":
            cpt_value = 1
            if separator in str(raw_value):
                value_list = list(str(raw_value).split(separator))
                for value in value_list:
                    container_dict[str(attribute + "_" + str(cpt_value))] = str(make_into_encoding(value))
                    cpt_value += 1
            else:
                container_dict[str(attribute)] = str(make_into_encoding(raw_value))
    return container_dict

def dataframe_from_md5_and_id_unik(db_package, limit = "", only_id = True):
    if limit:
        id_query = "SELECT mail_id, prenom, nom, civilite, birth, cp, ville FROM id_unik LIMIT %s ORDER BY mail_id ASC;" \
                   % str(limit)
    else:
        id_query = "SELECT mail_id, prenom, nom, civilite, birth, cp, ville FROM id_unik ORDER BY mail_id ASC;"
    if only_id:
        md5_query = "SELECT base.id AS mail_id, base.mail, md5.md5 FROM base " + \
                    "LEFT JOIN md5 ON base.id = md5.mail_id " + \
                    "RIGHT JOIN id_unik ON base.id = id_unik.mail_id ORDER BY mail_id ASC;"
    else:
         md5_query = "SELECT base.id AS mail_id, base.mail, md5.md5 FROM base " + \
                    "LEFT JOIN md5 ON base.id = md5.mail_id ORDER BY mail_id ASC;"
    initiate_threaded_connection_pool(db_package)
    with getconnection() as db_connect:
        id_df = pd.read_sql(id_query, db_connect, coerce_float=False)
    show_df(id_df)
    with getconnection() as db_connect:
        md5_df = pd.read_sql(md5_query, db_connect, coerce_float=False)
    show_df(md5_df)
    if only_id:
        df = pd.merge(id_df, md5_df)
    else:
        df = pd.merge(md5_df, id_df, how = 'left')
    #df.sort(['mail_id'])
    show_df(df)
    return df

def insert_dataframe_into_redis(df, max_record = 100000, max_pipe = 50000, json_mod = True, zip_mod = False):
    attribute_list = ['civilite', 'prenom', 'nom', 'cp', 'ville', 'birth']
    record_list = []
    cpt_record = 0
    cpt_record_max = 0
    for cpt in range(len(df.index)):
        if cpt_record >= 31600000:
            cpt_row = df.index[cpt]
            data = {}
            data['mail_id'] = df.at[cpt_row, 'mail_id']
            data['mail'] = df.at[cpt_row, 'mail']
            id = {}
            for attribute in attribute_list:
                id = unpack_id_attribute(id, attribute, df.at[cpt_row, attribute])
            if id:
                if json_mod:
                    id = json.dumps(id, sort_keys = True)
                    if zip_mod:
                        ...
                        out.close()
                data['id'] = id
            md5 = df.at[cpt_row, 'md5']
            record = {}
            record[md5] = data
            record_list.append(record)
            cpt_record_max +=1
        cpt_record +=1
        if cpt_record_max == max_record:
            with open("/home/david/python/record_list_to_redis.obj", "wb") as output_file:
                cPickle.dump(record_list, output_file)
            for record in record_list[:2]:
                print record
            write_records_in_redis(record_list, pipe_max = max_pipe)
            print "-- record list executed with %s records | now at record number %s" % (str(max_record), str(cpt_record))
            for record in record_list[-2:]:
                print record
            record_list = []
            cpt_record_max = 0
    write_records_in_redis(record_list, pipe_max)
    print "-- record list executed with %s records | now at record number %s" % (str(max_record), str(cpt_record))
    for record in record_list[-2:]:
        print record

def write_records_in_redis(record_list, pipe_max = 20000):
    ...
    for record in record_list:
        ...
        if record:
            ...
            pipe.hset(md5, 'mail_id', data['mail_id'])
            pipe.hset(md5, 'mail', data['mail'])
            if 'id' in data.keys():
                if data['id']:
                    pipe.hset(md5, 'id', data['id'])
                    #id = data['id']
                    #if json_mod:
                    #    id = json.loads(id, encoding='utf-8')
                    #for key, value in id.iteritems():
                    #    pipe.hset(md5, key, value)
        cpt_pipe += 1
        if cpt_pipe == pipe_max:
            cpt_pipe = 0
            pipe.execute()
            print "pipe executed with %s records | now at record number %s" % (str(pipe_max), str(cpt_record))
    pipe.execute()
    pipe.reset()
